...
export const TasksPage: React.FC<TasksPageProps> = ({
  ...
  const TaskCard = ({
    ...
    return <Card className="glass-card p-4 hover:shadow-lg transition-all duration-300 py-[6px] px-[6px]">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center ${task.completed ? 'bg-green-100 text-green-600' : 'bg-princess-pink/20 text-princess-pink'}`}>
            {task.completed ? <CheckCircle className="w-6 h-6" /> : <Icon className="w-6 h-6" />}
          </div>
  ...
  const CategoryProgress = ({
    ...
    return <div className="mb-4">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div className="bg-gradient-to-r from-princess-pink to-princess-purple h-2 rounded-full transition-all duration-300" style={{
  ...
  return <div className="space-y-6">
      ...
      <Card className="glass-card p-4">
        <h2 className="font-bold text-lg cursor-pointer select-none text-center" onClick={handleHeaderTap}>
          مركز المهام
        </h2>
      </Card>
      ...
      <Tabs defaultValue="daily" className="w-full" dir="rtl">
        ...
        <TabsContent value="daily" className="space-y-4">
          ...
          <div className="space-y-4">
            {getTasksByCategory('daily').map(task => <TaskCard key={task.id} task={task} />)}
          </div>
        </TabsContent>
        ...
        <TabsContent value="main" className="space-y-4">
          ...
          <div className="space-y-4">
            {getTasksByCategory('main').map(task => <TaskCard key={task.id} task={task} />)}
          </div>
        </TabsContent>
        ...
        <TabsContent value="partner" className="space-y-4">
          ...
          <div className="space-y-4">
            {getTasksByCategory('partner').map(task => <TaskCard key={task.id} task={task} />)}
          </div>
        </TabsContent>
      </Tabs>
    </div>;
